...
class TalentoHulpSupabaseUser extends BaseAuthUser {
  TalentoHulpSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> talentoHulpSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TalentoHulpSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
